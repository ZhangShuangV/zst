...
command.command('bundle').action(function () { //命令模块，输入 zs bundle 执行 bundle()方法，打包;
    bundle();
});
...
function bundle() { //合并压缩css、js并添加时间戳，移动index.html文件
    ...
    fs.writeFileSync('./build/'+cssName.slice(1), uglifiedCss); //将合并的css文件写入bundle.css
    console.log('生成bundle.css文件');
    var jsName = "./bundle." + new Date().getTime() + ".js";
    fs.writeFileSync('./build/'+jsName.slice(1), uglifiedJs.code);
...
function handleDir(dirpath) { //删除原有目录，生成新目录
    if(fs.existsSync(dirpath)) { //监测目录是否存在
        ...
        console.log('删除'+dirpath.slice(2)+'成功');
